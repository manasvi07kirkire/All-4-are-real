...
const navLinks = [
  ...
  { href: "/regression/184", label: "Regression", shortLabel: "REG", icon: AlertTriangle },
  { href: "/geo", label: "GEO Engine", shortLabel: "GEO", icon: Compass },
  { href: "/graph", label: "Crawler Graph", shortLabel: "GRAPH", icon: Network },
  { href: "/remediation", label: "Remediation & Gate", shortLabel: "FIX", icon: CheckCircle },
  { href: "/seo-advisor", label: "SEO Advisor", shortLabel: "SEO", icon: Sparkles },
];
      ...
      <header className="w-full bg-bone-100 border-b border-bone-300 sticky top-0 z-50 shadow-card">
        <div className="w-full px-4 sm:px-6 py-0 flex items-center justify-between gap-3 h-14">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden lg:inline text-bone-300 font-mono text-base">|</span>

            <span className="hidden lg:inline text-xs font-sans text-bone-700">
              Target: <code className="text-ink-900 font-semibold font-mono bg-bone-300/40 px-1.5 py-0.5 rounded-sm">acme/precision-store</code>
            </span>
          </div>
          ...
          <nav className="hidden lg:flex items-center h-full gap-0.5 font-mono text-xs">
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <link.icon className={clsx("w-3.5 h-3.5 shrink-0", active ? "text-ember-600" : "text-bone-500")} />
          ...
          <div className="flex items-center gap-2">
            ...
            {onTriggerPoisonedDeploy && (
              ...
              >
                ...
                <span className="hidden md:inline">{isTriggering ? "SIMULATING..." : "SIMULATE POISON"}</span>
              </button>
            )}
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-bone-300">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono font-bold text-base text-ink-900">SEARCHOPS</span>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-1">
          <span className="px-3 py-1 font-mono text-2xs font-bold text-bone-500 uppercase tracking-widest">Navigation</span>
          {navLinks.map((link) => {
            const active = isActive(link.href);
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon className={clsx("w-4.5 h-4.5 shrink-0", active ? "text-ember-600" : "text-bone-500")} />
                  <span>{link.label}</span>
                </div>
                <ChevronRight className={clsx("w-4 h-4 shrink-0", active ? "text-ember-600" : "text-bone-400")} />
              </Link>
            );
          })}
        </nav>
        ...
        <div className="px-4 py-4 border-t border-bone-300 flex flex-col gap-2">
          {onSelectDeploy && (
            <div className="flex items-center gap-1 bg-bone-300/40 p-1 rounded-sm border border-bone-300">
              {[183, 184, 185].map((num) => (
                <button
                  key={num}
                  onClick={() => { onSelectDeploy(num); setMobileOpen(false); }}
                  className={clsx(
          ...
          {onTriggerPoisonedDeploy && (
            <button
              onClick={() => { onTriggerPoisonedDeploy?.(); setMobileOpen(false); }}
              disabled={isTriggering}
          ...
          <nav className="flex flex-col gap-0.5">
            {links.map((link) => {
              ...
              return (
                ...
                >
                  <Icon className={clsx("w-4 h-4 shrink-0", active ? "text-ember-600" : "text-bone-500")} />
        ...
        <div className="flex flex-col gap-3 bg-bone-300/30 p-4 rounded-sm border border-bone-300">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center justify-between">
              <span className="font-sans text-sm font-semibold text-ink-900">1. Search Crawlers</span>
            ...
            <div className="flex items-center justify-between">
              <span className="font-sans text-sm font-semibold text-ink-900">2. AI Answer Engines</span>
