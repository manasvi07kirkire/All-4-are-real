...
const RadialGauge: React.FC<RadialGaugeProps> = ({ value, label, sublabel, previousValue }) => {
  ...
  const dashOffset = animated ? arcLength - (clamped / 100) * arcLength : arcLength;
  ...
  return (
    <div className="bg-ink-800 border border-ink-700 rounded-md p-5 flex flex-col items-center gap-4 shadow-card">
      ...
      <div className="w-full flex items-center justify-between border-b border-ink-700 pb-3">
        <span className="font-mono text-xs font-bold text-bone-500 uppercase tracking-widest">{label}</span>
        {delta !== undefined && (
          <span className={clsx(
            "font-mono text-sm font-black tabular-nums px-2.5 py-1 rounded-sm border",
            delta < 0 ? "text-ember-400 bg-ember-tint border-ember-600/40"
      ...
      <div className="relative w-full max-w-[220px]">
        <svg viewBox={`0 0 ${size} ${size * 0.88}`} className="w-full h-auto overflow-visible">
          <circle cx={center} cy={center} r={radius} fill="none" stroke="#15110D"
            strokeWidth={strokeWidth}
            strokeDasharray={`${arcLength} ${totalCircumference}`}
            strokeLinecap="round"
            transform={`rotate(${startAngle} ${center} ${center})`}
          />
          <circle cx={center} cy={center} r={radius} fill="none" stroke={strokeColor}
            strokeWidth={strokeWidth}
            strokeDasharray={`${arcLength} ${totalCircumference}`}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-3">
          <span className={clsx("font-display font-black tabular-nums leading-none", valueColor)}
            style={{ fontSize: "clamp(2.5rem, 4vw, 3.5rem)" }}>
            {clamped}
          </span>
          <span className="font-mono text-xs tracking-widest text-bone-500 uppercase mt-1">/ 100</span>
        </div>
      </div>

      {/* Status + Sublabel */}
      <div className="w-full text-center border-t border-ink-700 pt-3 flex flex-col items-center gap-2">
        <p className="font-sans text-sm font-semibold text-bone-300">{sublabel}</p>
        <div className={clsx(
...
export const RemediationRadialGauges: React.FC<RemediationRadialGaugesProps> = ({
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between border-b-2 border-bone-300 pb-3">
        <div className="flex items-center gap-3">
          <span className="font-mono text-sm font-black text-ink-900 uppercase tracking-wider">HEALTH RECOVERY</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-bone-100 border border-bone-300 rounded-sm p-4 flex items-center justify-between">
          <div>
            <span className="font-mono text-xs text-bone-700 uppercase tracking-wider block mb-0.5">Search Before</span>
            <span className="font-display font-black text-2xl text-ember-600 tabular-nums">{beforeSearch}</span>
          </div>
          <span className="font-mono text-2xl text-bone-400">→</span>
          <div className="text-right">
            <span className="font-mono text-xs text-bone-700 uppercase tracking-wider block mb-0.5">After</span>
            <span className="font-display font-black text-2xl text-patina-600 tabular-nums">{afterSearch}</span>
          </div>
        </div>

        <div className="bg-bone-100 border border-bone-300 rounded-sm p-4 flex items-center justify-between">
          <div>
            <span className="font-mono text-xs text-bone-700 uppercase tracking-wider block mb-0.5">GEO Before</span>
            <span className="font-display font-black text-2xl text-ember-600 tabular-nums">{beforeGeo}</span>
          </div>
          <span className="font-mono text-2xl text-bone-400">→</span>
          <div className="text-right">
            <span className="font-mono text-xs text-bone-700 uppercase tracking-wider block mb-0.5">After</span>
            <span className="font-display font-black text-2xl text-patina-600 tabular-nums">{afterGeo}</span>
          </div>
        </div>
      </div>
    </div>
