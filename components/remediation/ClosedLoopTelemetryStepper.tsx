...
import { CheckCircle, Clock, Activity, GitMerge, Search, RefreshCw } from "lucide-react";
...
const DEFAULT_STEPS: TelemetryStep[] = [
  {
    ...
    description: "AST parser flagged missing canonical emitter in ProductMetadata.tsx. 127 routes affected.",
    timing: "0.8ms",
    status: "done",
    icon: Activity,
  },
  {
    ...
    description: "Git blame isolated commit abc1234. Deterministic evidence: 0 regressions in deploy #183.",
    timing: "1.2ms",
    status: "done",
    icon: Search,
  },
  {
    ...
    description: "Tier-A auto-fix PR #185 created. Restores alternates.canonical across all product routes.",
    timing: "340ms",
    status: "done",
    icon: GitMerge,
  },
  {
    step: 4,
    label: "MERGE APPLIED",
    description: "PR #185 merged to main. CI/CD pipeline triggered. Build: PASSED.",
    timing: "12s",
    status: "active",
    icon: GitMerge,
  },
  {
    ...
    description: "Googlebot + AI engine re-verification in progress across all 127 routes.",
    timing: "~4min",
    status: "pending",
    icon: RefreshCw,
  },
...
export const ClosedLoopTelemetryStepper: React.FC<ClosedLoopTelemetryStepperProps> = ({
  steps = DEFAULT_STEPS,
  title = "CLOSED-LOOP REMEDIATION PIPELINE",
}) => {
  return (
          ...
          return (
            <div key={step.step} className="relative flex gap-4 pb-4">
              ...
              <div className={clsx(
                ...
                isDone ? "bg-patina-tint border-patina-400 text-patina-400"
                  : isActive ? "bg-ember-tint border-ember-400 text-ember-400 node-active"
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={clsx(
                      ...
                      isDone ? "text-patina-400" : isActive ? "text-ember-400" : "text-bone-500"
                    )}>
                      {step.label}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-mono text-xs font-bold text-ink-900 tabular-nums">{step.timing}</span>
                  </div>
                </div>
