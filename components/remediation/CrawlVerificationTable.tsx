...
const statusBadge = (code: number) => {
  if (code === 200) return <span className="font-mono text-xs font-bold text-patina-400 bg-patina-tint px-2 py-0.5 rounded-sm border border-patina-400/30">200 OK</span>;
  if (code >= 300 && code < 400) return <span className="font-mono text-xs font-bold text-steel-400 bg-ink-850 px-2 py-0.5 rounded-sm border border-ink-700">{code} REDIRECT</span>;
  if (code >= 400) return <span className="font-mono text-xs font-bold text-ember-400 bg-ember-tint px-2 py-0.5 rounded-sm border border-ember-400/30">{code} ERROR</span>;
  return null;
};
...
export const CrawlVerificationTable: React.FC<CrawlVerificationTableProps> = ({
  ...
  const passCount = rows.filter((r) => r.canonicalOk && r.status === 200).length;
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-bone-300/40 border-b border-bone-300">
              <th className="text-left font-mono text-xs font-black text-ink-900 uppercase tracking-wider px-4 py-3">Route</th>
              <th className="text-center font-mono text-xs font-black text-ink-900 uppercase tracking-wider px-4 py-3">HTTP</th>
              <th className="text-center font-mono text-xs font-black text-ink-900 uppercase tracking-wider px-4 py-3 hidden sm:table-cell">Canonical</th>
              <th className="text-center font-mono text-xs font-black text-ink-900 uppercase tracking-wider px-4 py-3 hidden md:table-cell">JSON-LD</th>
              <th className="text-center font-mono text-xs font-black text-ink-900 uppercase tracking-wider px-4 py-3">Indexing</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => {
              const allGood = row.canonicalOk && row.status === 200 && row.indexing === "YES";
              return (
                <tr
                  key={i}
                  className={clsx(
