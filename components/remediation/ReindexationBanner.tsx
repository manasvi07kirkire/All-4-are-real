    ...
    <div className={clsx(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className={clsx("font-mono text-sm font-black uppercase tracking-wider", statusConfig.color)}>
            {statusConfig.label}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-sans text-sm text-bone-700">
              <strong className="text-ink-900 font-mono">{routesReindexed.toLocaleString()}</strong>
              {" / "}
              <strong className="text-ink-900 font-mono">{totalRoutes.toLocaleString()}</strong>
              {" routes reindexed"}
            </span>
          </div>
          <span className={clsx("font-mono text-xl font-black tabular-nums", statusConfig.color)}>
            {pct}%
          </span>
        </div>
        <div className="w-full bg-bone-300/40 h-3 rounded-sm overflow-hidden">
          <div
            className={clsx("h-full rounded-sm transition-all duration-700",
              status === "COMPLETE" ? "bg-patina-400" :
                status === "IN_PROGRESS" ? "bg-marigold-400" : "bg-steel-400"
            )}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      {estimatedMinutes !== undefined && status === "IN_PROGRESS" && (
        <div className="flex items-center gap-2 font-sans text-sm text-bone-700">
          ...
          <span>Estimated completion: <strong className="text-ink-900">{estimatedMinutes} minutes</strong></span>
        </div>
      )}
      ...
      {status === "COMPLETE" && (
        <div className="flex items-center gap-2 font-sans text-sm text-patina-600 font-semibold">
          ...
          <span>All {totalRoutes} routes successfully reindexed. Health scores restored.</span>
        </div>
      )}
    </div>
  );
