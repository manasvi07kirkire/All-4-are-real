...
export const DeploymentStrip: React.FC<DeploymentStripProps> = ({
  ...
  return (
    <div className="w-full bg-ink-850 border border-line-600 rounded-md p-3 flex flex-col md:flex-row items-center justify-between gap-4">
      {/* Label */}
        ...
        {deployments.map((d, index) => {
          ...
          return (
            <React.Fragment key={d.id}>
              {index > 0 && (
                <ArrowRight className="w-3.5 h-3.5 text-line-500 shrink-0 hidden sm:block" />
              )}
              <button
                onClick={() => onSelectDeployment(d.deployNumber)}
                className={clsx(
                  "flex items-center gap-2.5 px-3 py-1.5 rounded-sm border transition-all text-left group",
                  isActive
                    ? "bg-ink-750 border-line-500 shadow-sm"
              ...
              >
                ...
                {/* Score */}
                <div className="flex items-center gap-1 pl-1 border-l border-line-600">
                ...
                {/* Commit info preview */}
                <div className="hidden lg:flex flex-col text-[10px] font-mono text-bone-500 pl-1">
